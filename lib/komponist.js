...
module.exports.getClient = function (mpdNamespace) {
  try { return komponists[mpdNamespace]; }
    ...
    delete komponists[mpdNamespace];
  }
};
...
var registerError = function (mpdNamespace, password) {
  komponists[mpdNamespace].on('error', function(err) {
    console.log(err);
    io_emitter.emit('clientError');
    delete komponists[mpdNamespace];
    // try reconnect
    /*var port = mpdNamespace.split(':')[1];
    var host = mpdNamespace.split(':')[0];
    var client = komponist.createConnection(port, host);
    client.once('ready', function() {
      authClient(client, password, mpdNamespace, false, function(err, obj) {
        if (err) {
          
        }
        else {
          komponists[mpdNamespace] = client;
          io_emitter.emit('reconnected');
        }
      });
    });*/
  });
  ...
  client.password(password, function(err, msg) { // authenticate with password
    if (err) { //authentication failed
      ...
      if (!komponistFound) { //only if client is new
        ...
        registerError(mpdNamespace, password);
