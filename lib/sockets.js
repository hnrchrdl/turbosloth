    ...
    if (socket.sessionID) {
      ...
      socket.on('mpd', function(cmd,args,callback) {
        redisClient.get('sessions:' + socket.sessionID, function(err, session) {
          ...
          var mpd_id = session.mpdhost + ":" + session.mpdport; // get mpd connection from session
          var komponistClient = komponist.getClient(mpd_id); // get komponist client
          if (komponistClient) {
            // fire mpd command
      ...
      socket.on('error', function(e) {
        console.log('debug socket error: ' + e.message)
      });
    }
  });
